...
public class RedisClient {
     ...
     * 设置永久Key-Value
     */
    private <T> void set(String keyPrefix, String keySuffix, T value) throws JsonProcessingException {
     ...
     * 设置会过期的key-value
     */
    public <T> void setWithExpiredTime(String keyPrefix, String keySuffix, T value, Long time, TimeUnit unit) throws JsonProcessingException {
     ...
     * 设置仅能设置一次的缓存
     */
    public <T> boolean setIfAbsent(String keyPrefix, String keySuffix, T value, Long time, TimeUnit unit) throws JsonProcessingException {
    ...
    public <V> V getWithNonHotSpotKey(
        ...
        if (obj == null && cacheNullObject) {
            // 将空值写入redis
            setWithExpiredTime(keyPrefix, keySuffix, "", CACHE_NULL_TTL, TimeUnit.MINUTES);
            // 返回错误信息
            return null;
        }
        // 存在，写入redis
        this.setWithExpiredTime(keyPrefix, keySuffix, obj, time, unit);
        return obj;
    }
